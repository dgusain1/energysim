...
- Typo in the variable name (e.g. ``'Gen1.p_MW'`` instead of
  ``'Gen1.p_mw'``).
- Element name does not match the name in the pandapower DataFrame
  (check ``net.gen['name']``, ``net.load['name']``, etc.).
...
- With ``coupling='jacobi'``, the loop is resolved with a one-step
  delay (each simulator sees the previous step's outputs).
- With ``coupling='gauss-seidel'``, Jacobi coupling is used within
  the loop.
- With ``coupling='iterative'``, the loop is iteratively resolved
  until convergence.
...
2. **Reduce ``t_macro``** — smaller exchange intervals make convergence
   easier.
3. **Check ``convergence_tolerance``** — ``1e-6`` may be too tight for
   some variables (e.g. large power values).  Try ``1e-4``.
4. **Ensure adapters support state save/restore** — if ``save_state()``
   returns ``None``, the adapter cannot be rolled back and iterative
   coupling degrades to Gauss-Seidel.
...
- **FMPy** — required for FMU simulators.  Install with
  ``pip install energysim[fmu]`` or ``pip install fmpy``.
- **pandapower** — required for powerflow simulators.  Install with
  ``pip install energysim[powerflow]`` or ``pip install pandapower``.
- **PyPSA** — required for PyPSA network simulators.  Install with
  ``pip install energysim[pypsa]`` or ``pip install pypsa``.
- **Plotly** — required for interactive HTML dashboards.  Install with
  ``pip install plotly``.
- **PowerFactory** — requires DIgSILENT PowerFactory 2020+ with its
  Python API enabled.  No extra pip package is needed; point ``pf_path``
  to the PowerFactory Python directory if it is not already on
  ``sys.path``.
- **MATLAB** — requires the MATLAB Engine for Python.  Install it from
  your MATLAB installation::

      cd "C:\Program Files\MATLAB\R2024b\extern\engines\python"
      python setup.py install

- **Octave** — requires GNU Octave on PATH and the ``oct2py`` bridge::
...
1. The ``sim_loc`` path is incorrect.  Use an absolute path or ensure
   the working directory contains the ``.m`` file.
2. The file is a **script**, not a **function**.  The first non-comment
   line must be ``function [...]`` — ``nargin`` only works on functions.
...
| Mode                | Best for                                           |
+=====================+====================================================+
| ``'jacobi'``        | Loosely coupled systems, simple setups,            |
|                     | backward compatibility                             |
+---------------------+----------------------------------------------------+
| ``'gauss-seidel'``  | Feed-forward chains, systems with a clear          |
|                     | upstream → downstream flow                         |
+---------------------+----------------------------------------------------+
| ``'iterative'``     | Tightly coupled systems with algebraic loops,      |
|                     | when accuracy of coupled variables is critical     |
+---------------------+----------------------------------------------------+
