# -*- coding: utf-8 -*-
"""Neural-network PV generation predictor — external simulator for energysim.

Loads pre-trained weights from pv_weights.npz and evaluates
...
class external_simulator(SimulatorAdapter):
    """Predicts PV generation (MW) from time-of-day and weather features."""
    ...
    def __init__(self, inputs=None, outputs=None, **kwargs):
        ...
        w = np.load(Path(__file__).resolve().parent.parent / "pv_weights.npz")
    ...
    def step(self, time):
        x = np.array([[self.hour_sin, self.hour_cos,
                        self.cloud_cover, self.temperature]])
